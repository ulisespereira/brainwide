...
class HebbianConnectivity:
    ...
    def __init__(self, modelparams, con_params):
        ...
        patterns_current_asym_1 = 2 * np.random.binomial(1, self.prob_pat, size=(self.n_ctx, self.N, self.p_asym)) - 1
        self.patterns_current_asym = np.stack((patterns_current_asym_1, patterns_current_asym_1), axis = 2)
        print('patterns_current_asym', self.patterns_current_asym.shape)
    ...
    def hebbian_projections(self, ind_post, ind_pre):
        ...
        for l in range(self.p):
            ...
            if (i in ind) * (j in ind):
                amp_ind_sym = self.amp_indexes[l][0][i,j]
                amp_ind_asym = self.amp_indexes[l][1][i,j]
            else:
                amp_ind_sym = 0
                amp_ind_asym = 0
            ...
            if i == j:
                ...
                hebb_sym += (amp_loc * amp_ind_sym/self.N) * np.outer(pat_post_sym, pat_pre_sym)
                ...
                hebb_sym += (amp_lr * amp_ind_sym/self.N) * np.outer(pat_post_sym, pat_pre_sym)
